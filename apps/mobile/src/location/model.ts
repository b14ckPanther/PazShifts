...
export function rankedRegions(
  ...
  return context.stations
    .filter(
      (s) =>
        (prefs.exit && s.id === context.activeStation) ||
        (prefs.arrival && (s.id === current || s.nextStart !== null))
    )
    .sort(
      (a, b) =>
        Number(b.id === context.activeStation) - Number(a.id === context.activeStation) ||
        (a.nextStart ?? Infinity) - (b.nextStart ?? Infinity) ||
    ...
    .map((s) => ({
      ...
      radius: s.radius,
      notifyOnEnter: prefs.arrival,
      notifyOnExit: prefs.exit,
    }));
}
export function shouldRemind(
  ...
  if (
    ...
    registry.expires <= now ||
    !prefs[kind] ||
    !registry.stations.includes(stationId)
  )
    return false;
  ...
  return kind === 'exit'
    ? context.activeStation === stationId
    : station.arrivalRelevant && context.activeStation === null;
}
