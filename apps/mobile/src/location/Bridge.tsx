...
export function LocationBridge() {
  ...
  useEffect(() => {
    ...
    const open = async (response: Notifications.NotificationResponse) => {
      ...
      try {
        if (
          data.userId !== worker.context.userId ||
          typeof data.stationId !== 'string' ||
          typeof data.createdAt !== 'number' ||
          Date.now() - data.createdAt > 86400000 ||
          data.createdAt > Date.now()
        )
          return;
        const fresh = await getNativeWorkerContext(supabase, worker.context.userId);
        if (!alive) return;
        if (!fresh.stations.some((s) => s.id === data.stationId)) {
          Alert.alert('התחנה אינה זמינה', 'ייתכן שההרשאה לתחנה השתנתה.');
          return;
        }
        worker.openStation(data.stationId, () => router.navigate('/'));
      } catch {
        if (alive) Alert.alert('לא הצלחנו לפתוח את התזכורת', 'בדקו את החיבור ונסו שוב.');
      } finally {
