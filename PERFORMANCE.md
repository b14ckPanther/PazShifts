...
## Mobile layout and streamed loading follow-up (2026-09-12)

- Worker hours: phone date fields use full-width rows and explicitly shrink native Safari date controls. Desktop retains the compact form. This addresses the iPhone screenshot without changing date/filter semantics.
- Shared worker/admin shell: reserve bottom-navigation space once, after the footer; remove duplicated mobile page minimum height/bottom padding. Footer remains in document flow close to content.
- Shared route fallback now includes the existing brand mark. CSS hides the footer while the route fallback exists and reveals it as content replaces the fallback; no hydration observer, extra request, cache, or timer is added. The existing startup splash still plays once per tab, skips NFC/reduced-motion, and does not block input. It is not a substitute for waiting on authoritative data.
- Verification: Chromium isolated layout fixtures using actual shared/worker CSS at 320, 393, 430, 768 and 1280 CSS px; no date overlap or horizontal overflow after changes. Gap from final report panel to footer card: 150px before → 40px after under identical fixture conditions. Footer hidden during a mounted route fallback and visible immediately after removal. Chromium did not reproduce Safari's original native-date overflow; physical Safari remains owner verification.
- Typecheck, lint, formatting, both production builds and all 57 existing Node tests passed. No measured reduction in hosted database/network latency is claimed by this follow-up. Existing request-local deduplication and report optimizations remain in place; no new private-data caching.
- Deploy both apps after committing. No new environment variables or database migration. Rollback is a revert of this layout/loading commit and redeployment.
