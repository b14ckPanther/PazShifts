...
1. Apply migrations in order, including `20260911000009_staff_permission_boundaries.sql` , `20260911000010_atomic_nfc_scans.sql`, `20260911000011_nfc_checkout_confirmation.sql`, and `20260911000012_auth_profile_contact_sync.sql`, to the intended Supabase project.
...
3. Confirm the worker has an active profile and active membership at the station, and check whether they already have a shift open. A scan with an existing active shift asks for checkout confirmation; it does not close the shift immediately.
...
- **Logged in, active shift at this station:** a fresh scan asks “לסיים את המשמרת?”. Confirm to clock out at confirmation time, or cancel to keep working. There is no checkout control outside the scan flow.
- **Logged out:** enter email or phone and the existing password; no OTP is sent. The same scan continues after authentication. Save and verify a worker phone through the admin edit form before trying phone login.
